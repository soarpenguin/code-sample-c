/***************************************************************************
* DESCRIPTION:
*   A simple hash table implementation for strings.
***************************************************************************/
 ...
 */
hash_node_t* hash_lookup(const hash_t *tptr, const char *key) {
    int h;
    hash_node_t *node;
 ...
 *  exists update the data, otherwise insert the entry and return HASH_SUCCESS.
 *
...
VMDEXTERNSTATIC int hash_insert(hash_t *tptr, const char *key, int data) {
    ...
    if ((node = hash_lookup(tptr, key)) != NULL) {
        node->data = data;
        return HASH_SUCCESS;
    }
...
VMDEXTERNSTATIC char * hash_stats(hash_t *tptr) {
    ...
    sprintf(buff, "%u slots, %u entries, and %1.2f ALOS",
            (int)tptr->size, (int)tptr->entries, alos(tptr));

    return(buff);
}

int hash_nsprintf(char *buf, size_t size, const char* fmt, ...) {
    int     i;
    va_list args;
...
/*
 *  hash_content() - Return a string with content about a hash table.
 *
 *  tptr: A pointer to the hash table
 */
VMDEXTERNSTATIC char * hash_content(hash_t *tptr) {
    ...
    for (i = 0; i < tptr->size; i++) {
        ...
        while (node != NULL) {
            last = node;
            node = node->next;
            j = hash_nsprintf(pointer, n, "%s:%d,", (char *)last->key, last->data);
            pointer += j;
            n -= j;
        }
    }
